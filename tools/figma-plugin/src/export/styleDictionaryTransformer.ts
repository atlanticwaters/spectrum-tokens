    ...
    if (components.length >= 3) {
      ...
      alpha =
        typeof raw.alpha === "number"
          ? raw.alpha
          : ((components[3] as number | undefined) ?? 1);
 ...
 */
export const transformNameAndroid = (
  tokenName: string,
  path: string[],
): string => {
 ...
 */
export const transformNamePascal = (
  tokenName: string,
  path: string[],
  prefix?: string,
): string => {
...
export const transformTypographyCompose = (value: unknown): string => {
  ...
  const fontFamily = getString(typographyValue.fontFamily)
    .toLowerCase()
    .replace(/ /g, "_");
...
export function transformToStyleDictionary(
  ...
  return transformToken(dtcgTokens) as Record<
    string,
    StyleDictionaryToken | Record<string, unknown>
  >;
}
